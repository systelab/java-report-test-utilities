...
class PDFVisualComparatorUtilTest {
  ...
  void shouldReturnTrueForIdenticalPdfs() throws IOException {
    ...
    assertThat(comparator.compare(expected, identical, target("diff_identical")))
        .as("Identical PDFs should be equal")
        .isTrue();
  }
  ...
  void shouldReturnFalseForDifferentPdfs() throws IOException {
    ...
    assertThat(comparator.compare(expected, different, target("diff_different")))
        .as("Different PDFs should not be equal")
        .isFalse();
  }
  ...
  void shouldExcludeRegionAndMatch() throws IOException {
    ...
    assertThat(comparator.compare(expected, different, target("diff_exclude_region")))
        .as("PDFs should be equal after excluding the region")
        .isTrue();
  }
  ...
  void shouldExcludeInputStreamAndMatch() throws IOException {
    ...
    final ByteArrayInputStream ignoreIS = new ByteArrayInputStream(
        "exclusions: [{page: 1,x1: 2280,y1: 100,x2: 2880,y2: 200},{page: 2,x1: 2280,y1: 100,x2: 2880,y2: 200},{page: 3,x1: 2280,y1: 100,x2: 2880,y2: 200}]".getBytes()
    );
    ...
    assertThat(comparator.compare(multiPageOnePage, multiPage, target("diff_exclude_page")))
        .as("PDFs should be equal after excluding pages")
        .isTrue();
  }
  ...
  void shouldExcludePagesAndMatch() throws IOException {
    ...
    assertThat(comparator.compare(multiPage, multiPageOnePage, target("diff_exclude_pages")))
        .as("PDFs should be equal after excluding pages")
        .isTrue();
  }
  ...
  void shouldApplyExclusionFile() throws IOException {
    ...
    assertThat(comparator.compare(multiPage, multiPageDiff, target("diff_exclusion_file")))
        .as("PDFs should be equal after applying the exclusion file")
        .isTrue();
  }
}